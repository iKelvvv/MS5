...
def all_products(request):
    ...
    if request.GET:
        ...
        if 'q' in request.GET:
            query = request.GET['q']
            if not query:
                messages.error(request, "Your search didn't match any criteria!")
...
def add_product(request):
    """ Add a product to the store via an admin panel """
...
def delete_product(request, product_id):
    ...
    product = get_object_or_404(Product, pk=product_id)
    product.delete()
    messages.success(request, 'Product has successfully deleted!')
